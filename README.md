...
- **물품 인식:**  
  ◦ YOLO Segmentation으로 클래스·마스크·중심점을 얻고, 마스크 내부 유효 depth의 중위값으로 카메라 기준 3D 좌표를 계산합니다.
- **품목 파지:**  
  ◦ Hand–Eye 보정 행렬과 로봇 자세로 좌표를 변환하고 마스크 형상 및 품목별 설정을 적용합니다.
  ◦ 
- **컵라면 전용 파지:**  
  ◦ 외부 FoundationPose–GraspGenX 파이프라인에 인식을 요청하고, 후보의 기울기·점수·접근 및 후퇴 경로를 검증하여 실행합니다.
- **충돌 환경 구성:**  
  ◦ 초기 관측 구간에서 필터링한 포인트클라우드로 정적 OctoMap을 생성하고 재사용합니다. 별도 keepout box도 설정합니다.
- **작업 관리:**  
  ◦ 상태머신으로 관찰·파지·배치·검사·재시도와 결과 보고를 제어합니다.
- **이력·재고 관리:**  
  ◦ MongoDB에 명령과 실행 결과를 기록하고 PostgreSQL에서 품목과 재고를 관리합니다.
